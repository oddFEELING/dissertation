...
def calc_cancer_score(state: IngestState, file_name: str, ):
    ...
    adata.obs['cancer_score'] = np.round(cancer_scores_norm, decimals=2)
    ...
    state.adata = adata
    file_save_path = Path(state.figures_dir) / file_name
    adata.write_h5ad(file_save_path)
    print(f'Saved file to {file_save_path}')
